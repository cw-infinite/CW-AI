...
async function rpc(server: Pick<McpServerConfig, 'url' | 'token'>, method: string, params?: Record<string, unknown>, signal?: AbortSignal, sessionId?: string): Promise<{ response: RpcResponse; sessionId?: string }> {
  ...
  const response = await fetch(validateUrl(server.url), { method: 'POST', signal, headers, body: JSON.stringify({ jsonrpc: '2.0', id: ++requestId, method, params }) });
  const text = await response.text();
  if (!response.ok) throw new Error(`${method} failed (${response.status}): ${text.slice(0, 300) || response.statusText}`);
...
export async function discoverMcpServer(server: McpServerConfig, signal?: AbortSignal): Promise<McpToolDefinition[]> {
  ...
  await rpc(server, 'notifications/initialized', undefined, signal, initialized.sessionId).catch(() => undefined);
  ...
  return (result.tools ?? []).filter(t => typeof t.name === 'string' && /^[a-zA-Z0-9_-]{1,64}$/.test(t.name)).map(t => ({
    name: t.name, description: t.description, inputSchema: t.inputSchema ?? { type: 'object', properties: {} }, readOnly: t.annotations?.readOnlyHint === true,
  }));
}
...
export function safeMcpServer(raw: Pick<McpServerConfig, 'id' | 'name' | 'url' | 'enabled' | 'token'>): McpServerConfig {
  return { ...raw, name: raw.name.trim().slice(0, 80) || 'MCP server', url: validateUrl(raw.url), tools: [] };
}
